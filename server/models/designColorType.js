...
const getDesignColorTypeByDesignType = async (data) => {
  try {
    const result = await prisma.$queryRaw`
    SELECT DISTINCT IdDesignColorType, DesignColorTypeDescription
    FROM DesignColorType
    INNER JOIN DesignColors ON DesignColorType.IdDesignColorType = DesignColors.DesignType_idDesignType
    WHERE DesignColors.DesignType_idDesignType = ${data.designType_idDesignType}
  `;
